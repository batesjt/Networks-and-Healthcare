...
// main_approx_ver2_wip.cpp -o main_approx_ver2_wip$(python3-config --extension-suffix)
...
vector<long double> h_motifs_count(string inputGraph)
{
	clock_t start;
	clock_t run_start;
	int progress;
	...
	return h_motif_final;
}

PYBIND11_MODULE(main_approx_ver2_wip, m) {
    m.doc() = "pybind11 example plugin"; // optional module docstring

    m.def("h_motifs_count", &h_motifs_count, "A function that adds two numbers");
}
